...
/*:
 * @target MZ
 * @plugindesc v1.0.2 ☆ Sistema de notas de atualização com interface medieval fantástica
...
(() => {
    ...
    const manifest = {
        ...
        init: context => {
            ...
            // Adicionar botão imediatamente e sempre que a tela de título aparecer
            setTimeout(() => addPatchNotesButton(), 500);
    ...
    function addPatchNotesButton() {
        ...
        const existingButton = document.querySelector('[data-command="patchnotes"]');
        if (existingButton) {
            return;
        }

        // Encontrar o container de comandos da tela de título
        const commandsContainer = document.querySelector('.as-title__commands');
        if (!commandsContainer) {
            logger.warn('Container de comandos não encontrado. Tentando novamente...');
            setTimeout(() => addPatchNotesButton(), 500);
            return;
        }

        // Criar botão usando a mesma estrutura dos botões existentes
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'as-title__button';
        button.setAttribute('data-command', 'patchnotes');
        
        button.innerHTML = `
            <img class="as-title__button-frame" src="js/plugins/assets/resources/botao.png" alt="" draggable="false" />
            <span class="as-title__button-text">Atualizações</span>
        `;
        ...
        // Inserir o botão antes do botão "Sair"
        const shutdownButton = commandsContainer.querySelector('[data-command="shutdown"]');
        if (shutdownButton) {
            commandsContainer.insertBefore(button, shutdownButton);
        } else {
            commandsContainer.appendChild(button);
        }
        
        logger.info('Botão de Patch Notes adicionado ao menu da tela de título.');
    }

    function removePatchNotesButton() {
        const button = document.querySelector('[data-command="patchnotes"]');
