...
/*:
 * @target MZ
 * @plugindesc v1.0.3 ☆ Sistema avançado de telemetria com agrupamento, performance e retry tracking
...
(() => {
    ...
    function setupAgent(context) {
        ...
        if (state.originalConsole && state.originalConsole.log) {
            state.originalConsole.log(`✓ [LogEnhancer] v${MODULE_VERSION} inicializado com recursos avançados`);
        }
        return api;
    }
    ...
    function dispose(state) {
        ...
        // Usar console original para evitar loop
        if (state.originalConsole && state.originalConsole.log) {
            state.originalConsole.log('🗑️ [LogEnhancer] Recursos liberados');
        }
    }
    ...
    function buildEntry(state, level, args, meta) {
        ...
        if (frame.performance.slowLog) {
            // Usar console original para evitar loop infinito
            if (state.originalConsole && state.originalConsole.warn) {
                state.originalConsole.warn(`⚡ [LogEnhancer] Log lento detectado: ${deltaSinceLast}ms desde último log`);
            }
        }
    ...
    function detectErrorPattern(state, group) {
        ...
        if (!exists) {
            ...
            // Usar console original para evitar loop
            if (state.originalConsole && state.originalConsole.warn) {
                state.originalConsole.warn(`🔁 [LogEnhancer] Padrão de erro detectado: "${group.message}" (${group.count}x)`);
            }
    ...
    function trackRetry(state, operationName) {
        ...
        // Usar console original para evitar loop
        if (state.originalConsole && state.originalConsole.info) {
            state.originalConsole.info(`🔁 [LogEnhancer] Retry #${counter.attempts} para "${operationName}"`);
        }
        return counter;
    }
    ...
    function trackSceneTransition(state, fromScene, toScene) {
        ...
        // NÃO usar console.info aqui - causa loop infinito!
        // Usar console original direto para evitar recursão
        if (state.originalConsole && state.originalConsole.info) {
            state.originalConsole.info(`🎬 [LogEnhancer] Transição: ${fromScene} → ${toScene}`);
        }
    }
    ...
    function detectMemoryLeak(state) {
        ...
        if (leaked) {
            // Usar console original para evitar loop
            if (state.originalConsole && state.originalConsole.warn) {
                state.originalConsole.warn(`🛑 [LogEnhancer] Possível memory leak detectado! +${(increase / 1024 / 1024).toFixed(2)}MB`);
            }
        }
