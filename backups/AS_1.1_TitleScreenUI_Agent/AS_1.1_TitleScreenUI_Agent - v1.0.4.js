...
/*:
 * @target MZ
 * @plugindesc [v1.0.3] Interface da Tela de Título - Ancient Souls
 * @author 
 ...
 *
 * @help
 * AS_1.1_TitleScreenUI_Agent [v1.0.3]
 * 
 * ============================================================================
 * Descrição:
 * ============================================================================
...
(() => {
    ...
    class TitleScreenUIManager {
        ...
        init() {
            ...
            // Escuta evento de cleanup temporário (para transições)
        ...
        createUI() {
            ...
            // Cria elementos da UI (apenas menu, título é sprite agora)
        ...
        injectStyles() {
            ...
            style.textContent = `
                ...
                /* Botão de Atualizações no canto superior esquerdo */
                .as-updates-button {
                    position: absolute;
                    top: 20px;
                    left: 20px;
                    ...
                    opacity: 0;
                    animation: ${AS.TitleScreenUI.animatedMenu ? 'fadeInButton 1s ease-out forwards' : 'none'};
                    animation-delay: 1.2s;
                    z-index: 100;
                }
                
                .as-updates-button:hover {
        ...
        createMenuElement() {
            ...
            // Cria botões do menu (horizontal: Iniciar, Continuar, Opções, Sair)
            const buttons = [
                { id: 'newGame', text: 'Iniciar', enabled: true },
                { id: 'continue', text: 'Continuar', enabled: DataManager.isAnySavefileExists() },
                { id: 'options', text: 'Opções', enabled: true },
                { id: 'exit', text: 'Sair', enabled: true }
            ];
            ...
            // Cria botão de Atualizações no canto superior direito
            const updatesButton = this.createUpdatesButton();
            this.container.appendChild(updatesButton);
            this.elements.set('button_updates', updatesButton);
        ...
        createUpdatesButton() {
            ...
            button.className = 'as-updates-button';
            button.textContent = 'Atualizações';
            button.dataset.action = 'updates';
            
            // Event listeners
            button.addEventListener('click', () => this.onButtonClick('updates'));
            button.addEventListener('mouseenter', () => this.onButtonHover('updates'));
        ...
        hideUI() {
            this.log('Ocultando interface temporariamente...');
