...
/*:
 * @target MZ
 * @plugindesc v1.1.3 ☆ Sistema completo de patch notes com acordeão, BGM preservada, expandir tudo e contadores
...
(() => {
    ...
    function createPatchNoteElement(note) {
        ...
        // Seções com novo visual
        if (note.sections && note.sections.length > 0) {
            note.sections.forEach(section => {
                const sectionElement = document.createElement('section');
                sectionElement.className = 'as-patchnote__section as-patchnote__section--enhanced';
                
                const sectionTitle = document.createElement('h4');
                sectionTitle.className = 'as-patchnote__section-title as-patchnote__section-title--enhanced';
                
                // Extrair emoji do título se existir
                const emojiMatch = section.title.match(/^([^\w\s]+)\s*/);
                if (emojiMatch) {
                    const iconSpan = document.createElement('span');
                    iconSpan.className = 'as-patchnote__section-icon';
                    iconSpan.textContent = emojiMatch[1];
                    sectionTitle.appendChild(iconSpan);
                    sectionTitle.appendChild(document.createTextNode(section.title.replace(emojiMatch[0], '')));
                } else {
