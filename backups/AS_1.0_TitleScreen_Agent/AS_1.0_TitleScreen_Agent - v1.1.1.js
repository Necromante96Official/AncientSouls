...
/*:
 * @target MZ
 * @plugindesc [v1.1.1] Gerenciador da Tela de Título - Ancient Souls
 * @author 
 ...
 * @max 2.00
 * @default 0.25
 * @desc Escala da logo do jogo (0.25 = 25% do tamanho original).
 *
 * @param LogoPositionX
 * @text Posição X da Logo (%)
 * @type number
 * @decimals 2
 * @min 0
 * @max 100
 * @default 50
 * @desc Posição horizontal da logo em porcentagem da largura da tela (50 = centro).
 *
 * @param LogoPositionY
 * @text Posição Y da Logo (%)
 * @type number
 * @decimals 2
 * @min 0
 * @max 100
 * @default 30
 * @desc Posição vertical da logo em porcentagem da altura da tela (30 = círculo da explosão).
 *
 * @help
 * AS_1.0_TitleScreen_Agent [v1.1.1]
 * 
 * ============================================================================
 * Descrição:
 * ============================================================================
...
(() => {
    ...
    AS.TitleScreen.logoScale = Number(parameters.LogoScale) || 0.25;
    AS.TitleScreen.logoPositionX = Number(parameters.LogoPositionX) || 50;
    AS.TitleScreen.logoPositionY = Number(parameters.LogoPositionY) || 30;
    ...
    class TitleScreenManager {
        ...
        openPatchNotes() {
            this.log('Abrindo tela de atualizações (Patch Notes)...');
            
            // Remove elementos HTML antes de trocar de cena
            this.cleanupHTMLElements();
            
            // Verifica se Scene_PatchNotes existe
            if (typeof Scene_PatchNotes !== 'undefined') {
                SceneManager.push(Scene_PatchNotes);
            } else {
                this.log('⚠️ Scene_PatchNotes não encontrada! Verifique se AS_1.4_PatchNotesScreen_Agent está ativo.');
            }
        }
    ...
    if (AS.TitleScreen.enabled) {
        ...
        Scene_Title.prototype.createCustomBackground = function() {
            ...
            // Carrega logo do jogo (ancient-souls.png) - ajustável via parâmetros
            ...
            logoImg.onload = () => {
                ...
                // Usa o parâmetro LogoScale para definir o tamanho
                const logoScale = AS.TitleScreen.logoScale;
                this._logoSprite.scale.x = logoScale;
                this._logoSprite.scale.y = logoScale;
                
                // Posiciona o logo usando os parâmetros LogoPositionX e LogoPositionY
                const scaledWidth = logoImg.width * logoScale;
                const scaledHeight = logoImg.height * logoScale;
                
                // Converte porcentagem para pixels
                const targetX = (Graphics.width * AS.TitleScreen.logoPositionX / 100);
                const targetY = (Graphics.height * AS.TitleScreen.logoPositionY / 100);
                
                // Centraliza a logo no ponto alvo
                this._logoSprite.x = targetX - (scaledWidth / 2);
                this._logoSprite.y = targetY - (scaledHeight / 2);
                
                this.addChild(this._logoSprite);
                this.log(`✓ Logo carregado: escala=${logoScale}, posX=${AS.TitleScreen.logoPositionX}%, posY=${AS.TitleScreen.logoPositionY}%`);
            };
            logoImg.onerror = () => {
