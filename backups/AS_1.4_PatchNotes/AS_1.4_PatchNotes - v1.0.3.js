...
/*:
 * @target MZ
 * @plugindesc v1.0.3 ☆ Sistema de notas de atualização com interface medieval fantástica
...
(() => {
    ...
    const manifest = {
        ...
        init: context => {
            ...
            // Escutar comando do botão de patch notes
            const commandSubscription = context.subscribe('titlescreen:ui:command', (data) => {
                if (data && data.command === 'patchnotes') {
                    logger.info('Comando de Patch Notes recebido.');
                    openPatchNotes();
                }
            });
