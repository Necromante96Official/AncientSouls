...
/*:
 * @target MZ
 * @plugindesc v1.4.2 ☆ Interface HTML da tela de título (layout medieval fantástico)
...
(() => {
    ...
    function monitorSceneLifecycle(context) {
        ...
        // Esconder a UI imediatamente quando qualquer comando da UI for disparado (antes da troca de cena)
        unsubscribeUiCommand = context.subscribe('titlescreen:ui:command', handleUiCommandDispatched);
    }
    ...
    function handleSceneReady() {
        ...
        // Garantir que o elemento esteja exibível
        showUiContainer();
        updateContinueState();
    ...
    function handleSceneTerminate() {
        ...
        hideUiContainer(true);
        detachKeyboardSupport();
    }

    function handleUiCommandDispatched(payload) {
        // Ao clicar em qualquer ação do menu, esconda a UI imediatamente
        // Isso evita que o overlay persista por um frame quando a cena muda.
        hideUiContainer(true);
    }
    ...
    function showUiContainer() {
        if (!rootElement) return;
        // Tornar visível ao entrar na Scene_Title
        rootElement.style.display = 'flex';
        // Opacidade e pointer-events são controlados pela classe .as-title--visible
    }

    function hideUiContainer(immediate = false) {
        if (!rootElement) return;
        rootElement.classList.remove('as-title--visible');
        rootElement.setAttribute('aria-hidden', 'true');
        if (immediate) {
            // Remover da renderização para não sobrepor outras cenas mesmo com z-index alto
            rootElement.style.display = 'none';
            rootElement.style.opacity = '0';
            rootElement.style.pointerEvents = 'none';
        } else {
            // Após a transição de opacidade, desligar a exibição
            const off = () => {
                rootElement.style.display = 'none';
                rootElement.removeEventListener('transitionend', off);
            };
            rootElement.addEventListener('transitionend', off);
        }
    }
    ...
    function destroyMarkup() {
        ...
        if (unsubscribeUiCommand) {
            unsubscribeUiCommand();
            unsubscribeUiCommand = null;
        }
        layoutTargets = {};
    }
    ...
    function executeCommandDirect(command) {
        ...
        switch (command) {
            ...
            case 'shutdown':
                try {
                    // Fade out de áudio MUITO MAIS SUAVE (4 segundos = 240 frames)
                    AudioManager.fadeOutBgm(240);
                    AudioManager.fadeOutBgs(240);
                    AudioManager.fadeOutMe(240);
                    
                    // Transição visual ULTRA suave com easing otimizado
                    if (rootElement) {
                        rootElement.style.transition = 'opacity 4s cubic-bezier(0.25, 0.46, 0.45, 0.94), transform 4s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
                        rootElement.style.opacity = '0';
                        rootElement.style.transform = 'scale(0.95)';
                        rootElement.style.filter = 'blur(15px)';
                    }
                    
                    // Fade out da tela toda COM BLUR ultra suave
                    ...
                    fadeOverlay.style.cssText = `
                        ...
                        opacity: 0;
                        transition: opacity 4s cubic-bezier(0.25, 0.46, 0.45, 0.94), backdrop-filter 4s cubic-bezier(0.25, 0.46, 0.45, 0.94);
                        z-index: 99999;
                        pointer-events: none;
                    ...
                    // Aplicar fade gradual ultra suave
                    requestAnimationFrame(() => {
                        fadeOverlay.style.opacity = '1';
                        fadeOverlay.style.backdropFilter = 'blur(25px)';
                    });
                    
                    // Reduzir volume MUITO gradualmente em passos menores (33ms = ~120 steps em 4s)
                    let volumeStep = 0;
                    ...
                    // Aguardar transição completar antes de sair (4.2 segundos para garantir)
                    setTimeout(() => {
                        clearInterval(volumeFade);
                        SceneManager.exit();
                    }, 4200);
